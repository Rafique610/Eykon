...
### 4. Generation Parameters & Prompt Strategy

Match the Phase 1 current settings (`src/assistant/prompt.py` and `src/assistant/llm.py`):
- `temperature`: `0.3f` (balances factual grounding with natural deductive reasoning)
- `max_output_tokens`: `256`
- **System Instruction**: 7 strict guidelines + 5 few-shot examples from `src/assistant/prompt.py`.
- **Prefix stripping**: Strip leading `Answer:` / `answer:` prefix if emitted by the model.
